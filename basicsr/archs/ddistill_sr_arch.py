...
class ESA(nn.Module):
    ...
    def forward(self, x):
        c1_ = (self.conv1(x))
        c1 = self.conv2(c1_)
        # Handle small spatial sizes (e.g. small license plates during validation where H < 7 or W < 7)
...
class DDistill_SR(nn.Module):
    def __init__(self, in_nc=3, out_nc=3, nf=56, num_modules=4, scale=4, upscale=None, lite=False, deploy=True, dynamic=True, L=16, style='DBB', res=True, **kwargs):
        super(DDistill_SR, self).__init__()
        assert res == True
        assert deploy == True

        self.scale = upscale if upscale is not None else scale 
