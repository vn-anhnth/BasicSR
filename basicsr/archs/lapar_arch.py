# LAPAR: Linearly-Assembled Pixel-Adaptive Regression Network (NeurIPS 2020)
# Official repository: https://github.com/JIA-Lab-research/Simple-SR
...
class WeightNet(nn.Module):
    def __init__(self, in_chl=3, nf=32, n_block=4, out_chl=72, scale=2):
...
class ComponentDecConv(nn.Module):
    def __init__(self, k_path, k_size):
        ...
        if not os.path.isabs(k_path):
            ...
            alt_path = os.path.join(curr_dir, 'lapar_kernels', os.path.basename(k_path))
...
class LAPAR(nn.Module):
    """
    Linearly-Assembled Pixel-Adaptive Regression Network (LAPAR)
    Supports:
      - LAPAR-A: n_channel=32, res_block=4 (~659K params)
      - LAPAR-B: n_channel=24, res_block=3 (~380K params)
      - LAPAR-C: n_channel=16, res_block=2 (~190K params)
    """
    def __init__(
        self,
        scale=2,
        ...
        kernel_size=5,
        n_weight=72,
        kernel_path='basicsr/archs/lapar_kernels/kernel_72_k5.pkl'
    ):
        super(LAPAR, self).__init__()
        self.k_size = kernel_size
        self.s = scale

        self.w_conv = WeightNet(
            in_chl=in_channels,
            nf=n_channel,
            n_block=res_block,
            out_chl=n_weight,
    ...
    def forward(self, x):
        ...
        x_com = self.decom_conv(x_pad).view(B, 3, -1, self.s * H, self.s * W)

        weight = self.w_conv(x)
        weight = weight.view(B, 1, -1, self.s * H, self.s * W)
