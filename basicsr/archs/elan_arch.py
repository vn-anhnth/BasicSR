# ELAN: Efficient Long-Range Attention Network for Image Super-resolution (ECCV 2022)
# Official repository: https://github.com/xindongzhang/ELAN
...
class LFE(nn.Module):
    ...
    def __init__(self, inp_channels, out_channels, exp_ratio=4, act_type='relu'):
        ...
        if self.act_type == 'linear':
            ...
            raise ValueError(f'Unsupported type of activation: {self.act_type}')
...
class GMSA(nn.Module):
    ...
    def __init__(self, channels, shifts=4, window_sizes=[4, 8, 16], calc_attn=True):
...
class ELAB(nn.Module):
    """Efficient Long-range Attention Block"""
    def __init__(self, inp_channels, out_channels, exp_ratio=2, shifts=0, window_sizes=[4, 8, 16], shared_depth=1):
        ...
        for i in range(shared_depth):
            modules_lfe[f'lfe_{i + 1}'] = LFE(inp_channels=inp_channels, out_channels=out_channels, exp_ratio=exp_ratio)
            modules_gmsa[f'gmsa_{i + 1}'] = GMSA(channels=inp_channels, shifts=shifts, window_sizes=window_sizes, calc_attn=False)
    ...
    def forward(self, x):
        ...
        for i in range(1 + self.shared_depth):
            if i == 0:  # calculate attention for the 1st module
...
class ELAN(nn.Module):
    def __init__(
        ...
        # Mean normalization buffers
        self.register_buffer('rgb_mean', torch.Tensor(rgb_mean).view(1, 3, 1, 1) * rgb_range)
